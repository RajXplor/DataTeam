  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" onClick={onClose}>
      <div className="relative max-h-[90vh] max-w-[95vw] rounded-xl bg-white p-3 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <button type="button" onClick={onClose} className="absolute -top-3 -right-3 flex h-8 w-8 items-center justify-center rounded-full bg-slate-800 text-lg font-bold text-white shadow-lg hover:bg-slate-700" aria-label="Close enlarged image">×</button>
        <img src={src} alt="Enlarged screenshot" className="max-h-[86vh] max-w-[92vw] rounded-lg object-contain" />
      </div>
    </div>
  );
}
            ...
            <div className="grid sm:grid-cols-3 gap-5 mb-4">
              ...
              <div>
                <FileDropzone
                  ...
                  compact
                />
                <PathHint text="This is the report extracted using Azure runbook. Before inserting here, ensure that it is in the import template format." />
