...
Required arguments (prompted if not supplied via command line):
  - NEW Service ID
  - NEW Service Name
...
def process(
    ...
    for _, c in ch.iterrows():
        ...
        def ec_field(n, field):
            """Get field from EC slot n (1-based). Returns '' if no such EC."""
            ...
            if field == "Suburb":
                return _v(row.get("Emergency Contact Suburb", ""))
            if field == "Postcode":
                return ""  # EC report doesn't include postcode
            if field == "Country":
                return _v(row.get("Emergency Contact Country", ""))
            if field == "State":
                return _v(row.get("Emergency Contact State", ""))
            if field == "Email":
                return _v(row.get("Emergency Contact Email", ""))
            if field == "Emergency_Contact":
                return _ec_perm(row.get("Emergency Contact", "no"))
            if field == "Medical_Nominee":
                return _ec_perm(row.get("Medical", "no"))
            if field == "Collection_Nominee":
                return _ec_perm(row.get("Collection", "no"))
            if field == "Excursion_Nominee":
                return _ec_perm(row.get("Excursion", "no"))
            return ""
        ...
        row_data = {
            ...
            **{f"EmergencyContact{n}_{f}": ec_field(n, f)
               ...
               for f in [
                   ...
                   "Emergency_Contact", "Medical_Nominee", "Collection_Nominee",
                   "Excursion_Nominee",
               ]},
        }
...
# ──────────────────────────────────────────────────────────────────────────────────────
    ...
    if missing_gender_rows:
        print(f"\n  ⚠  WARNING: {len(missing_gender_rows)} children have no Gender set.")
        print("     Open PC_import.csv and type Male or Female in column G for these rows:\n")
        print(f"     {'Cell':<10} {'Child Name'}")
        print(f"     {'-'*38}")
        for idx in missing_gender_rows:
            csv_row = idx + 2
            name = f"{import_df.at[idx, 'Child_First_Name']} {import_df.at[idx, 'Child_Last_Name']}"
            print(f"     G{csv_row:<9} {name}")
...
===========================================================
  ...
  Children processed     : {len(import_df)}
  EC persons assigned    : {len(ec_legacy_id_map)}
  Children with ≥1 EC    : {ec_count}
  Missing Gender (review): {len(missing_gender)}

  Output saved to:
    {output_path}

  Next steps:
    1. Open PC_import.csv
    2. Fill in any blank Gender cells (Column G)
    3. Verify postcode columns look correct (4 digits)
    4. Save as CSV UTF-8
...
def main():
    ...
    parser.add_argument("--children",     default="Children_data.csv",
                        help="Path to Children data CSV (default: Children_data.csv)")
    parser.add_argument("--ec",           default="Emergency_contacts_data.xlsx",
                        help="Path to Emergency Contacts file (default: Emergency_contacts_data.xlsx)")
    ...
    parser.add_argument("--output",       default="PC_import.csv",
...
if __name__ == "__main__":
    ...
    sid  = input("  Enter NEW Service ID   : ").strip()
    snam = input("  Enter NEW Service Name : ").strip()
    print()

    process(
        children_path    = r"Children_data.csv",
        ec_path          = r"Emergency_contacts_data.xlsx",
        new_service_id   = sid,
        new_service_name = snam,
